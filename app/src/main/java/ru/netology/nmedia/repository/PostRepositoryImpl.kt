    ...
    override suspend fun likeById(id: Long) {
        try {
            ...
            if (post.likedByMe) {
                ...
                val responseDislike = Api.service.dislikeById(id)
                if (!responseDislike.isSuccessful) {
                    throw ApiError(responseDislike.code(), responseDislike.message())
                }
            } else {
                val likedPost = post.copy(likedByMe = true, likes = post.likes + 1)
                dao.insert(PostEntity.fromDto(likedPost))
                val responseLike = Api.service.likeById(id)
                if (!responseLike.isSuccessful) {
                    throw ApiError(responseLike.code(), responseLike.message())
                }
            }
        } catch (e: IOException) {
            throw NetworkError
